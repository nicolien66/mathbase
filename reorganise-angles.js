/* Polymates — reorganise-angles.js
   Renomme une famille du chapitre « Angles et parallélisme ».

        Prouver un parallélisme  →  Angles correspondants

   ⚠ LECTURE DE LA CONSIGNE
   La demande était : « renomme prouver un parallélisme par angles
   correspondants ». Deux lectures étaient possibles ; j'ai retenu celle-ci,
   la plus directe : l'ancien nom « Prouver un parallélisme » devient
   « Angles correspondants ».
   Si vous vouliez au contraire ALLONGER le nom en « Prouver un parallélisme
   par angles correspondants », il suffit de changer la ligne `apres` du
   tableau RENOMMAGES ci-dessous — rien d'autre. La simulation montre le
   résultat avant toute écriture.
   ...
   Usage :
     ...
     node reorganise-angles.js              # simulation
     node reorganise-angles.js --libelles   # + libellés exacts
     node reorganise-angles.js --execute
*/
...
const RENOMMAGES = [
  { avant: ["Prouver un parallélisme", "Prouver le parallélisme",
            "Démontrer un parallélisme", "Prouver un parallelisme"],
    /* ← la seule ligne à changer si le nom voulu était plus long */
    apres: "Angles correspondants" },
];
...
(async () => {
  ...
  const { rows } = await pool.query(
    `SELECT id, title, chapitre, famille FROM exercises
      WHERE chapitre ILIKE '%angle%' OR chapitre ILIKE '%parall%' ORDER BY id`);
  if (!rows.length) {
    console.log("Aucun exercice dans le chapitre « Angles et parallélisme ».");
  ...
  const nom = `angles-avant-renommage-${stamp}.json`;
  ...
  const { rows: bilan } = await pool.query(
    `SELECT chapitre, COALESCE(famille,'(sans famille)') AS famille, count(*)::int AS n
       FROM exercises WHERE chapitre ILIKE '%angle%' OR chapitre ILIKE '%parall%'
